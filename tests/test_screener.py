...
from src.utils.rate_limiter import reset_rate_limiters
...
def test_generate_url(screener):
    assert (
        screener.generate_url("RELIANCE")
        == "https://www.screener.in/company/RELIANCE/consolidated/"
    )
    assert (
        screener.generate_url("RELIANCE", consolidated=False)
...
def test_scrape_mock(mock_bs, mock_read_html, mock_get, screener):
    ...
    mock_soup.find.return_value.find.return_value.get_text.return_value = (
        "About text"  # about
    )
