...
from src.utils import console, track, get_list_size
from src.utils import MongoDBHandler
from src.data.utils import get_exchange_from_url, extract_text_from_pdf
import pandas as pd
...
class DataManager(object):
    ...
    def extract(self, url: str):
        ...
        if not exchange:
            console.log(f"[red]Could not extract exchange from doc url.")
    ...
    def download_results(
        ...
        # processing dates
        df["toDate"] = pd.to_datetime(df["toDate"], format="%d-%b-%Y", errors="coerce", dayfirst=True)
        ...
        data_list = []
        for item in track(xbrls_dict, description="Downloading financials [xbrls & htmls] ...", total = len(xbrls_dict)):
            url = item['xbrl']
        ...
        console.log(f"Data size: {size['b']} b, {size['kb']:.2f} kb, {size['mb']:.2f} mb")
    ...
    def download_shareholdings(
        ...
        df["date"] = pd.to_datetime(df["date"], format="%d-%b-%Y", errors="coerce", dayfirst=True)
        ...
        data_list = []
        for item in track(xbrls_dict, description="Downloading shareholdings ...", total = len(xbrls_dict)):
            url = item['xbrl']
