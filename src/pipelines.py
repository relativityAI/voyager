...
from src.utils.helpers import get_curr_date_str, get_date_shifted
...
def fetch_yf_symbols_prices(
    symbols_map: List[Dict], from_date: str = None, to_date: str = None
) -> pd.DataFrame:
    """
    Args:
    ...
    prices_df = yf.download(
        symlist, start=from_date, end=to_date, progress=False, auto_adjust=True
    )
    return prices_df
...
def fundamentals_pipeline(
    ...
    for symbol in symbols:
        ...
        fundamentals_df = calc_fundamental_ratios(
            processed_results_df.copy(), period=period
        )
...
def valuations_pipeline(
    ...
    for symbol in symbols:
        ...
        symbol_prices_df = get_symbol_prices(
            prices_df, generate_yf_symbol(symbol, exchange)
        )
        symbol_prices_df = symbol_prices_df.rename("price")
        symbol_prices_df = symbol_prices_df.rename_axis("date")

        valuation_df = calc_valuations_time_series(
            processed_results_df_q, symbol_prices_df
        )
...
def ratios_pipeline(
    ...
    for symbol in track(
        symbols, total=len(symbols), description="Calculating ratios..."
    ):
        # FETCHING RAW DATA
        df_q = fetch_raw_results(
            symbol=symbol,
