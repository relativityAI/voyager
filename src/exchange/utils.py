...
def get_fake_user_agent():
    try:
        ...
        console.log("[bold red]Failed to get fake user agent")
        console.log(f"[red]{e}")
        return None
...
def parse_xbrl(xbrl_content: BytesIO, get_json=False, display=False):
    if display:
        console.log("🔍 Parsing XBRL content...")
...
def parse_html_nse(response: requests.Response):
    ...
    def fuzzy_replace(value, mapping_dict):
        match, score = process.extractOne(
            str(value), mapping_dict.keys(), scorer=fuzz.token_set_ratio
        )
        return mapping_dict[match] if score >= match_threshold else value

    df[first_col] = df[first_col].apply(
        lambda x: fuzzy_replace(x, replacement_dict)
    )  # convert terminology
    df = df[
        pd.to_numeric(df[second_col], errors="coerce").notnull()
