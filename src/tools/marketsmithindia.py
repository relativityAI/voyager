...
class MarketSmithIndia:
    ...
    def fetch(self, symbol: str):
        ...
        result = {
            ...
            "EPS Growth Rate": f"{block.get('epsGrowthRate')}%" if block.get("epsGrowthRate") is not None else None,
            "Earnings Stability": block.get("earningsStability"),
            "P/E Ratio": header.get("pe"),
            "5-Year P/E Range": f"{block.get('pe5YearLow')}- {block.get('pe5YearHigh')}" if block.get("pe5YearLow") is not None else None,
            "Return on Equity": f"{header.get('roe')}%" if header.get("roe") is not None else None,
            "Cash Flow (INR)": round(block.get("cash_flow"), 2) if block.get("cash_flow") is not None else (round(block.get("cashFlow"), 2) if block.get("cashFlow") is not None else None)
        }
