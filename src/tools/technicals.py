...
class Technicals(object):
    ...
    def technicals(self, symbol, force=False):
        ...
        if not os.path.exists(technicals_file) or force:
            ...
            macd, macdsignal, macdhist = talib.MACD(latest_df['Close'], fastperiod=12, slowperiod=26, signalperiod=9)
            latest_df['MACD'] = macd
            latest_df['MACDSignal'] = macdsignal
            latest_df['MACDHist'] = macdhist

            # Example 5: Bollinger Bands
            upperband, middleband, lowerband = talib.BBANDS(latest_df['Close'], timeperiod=20, nbdevup=2, nbdevdn=2)
