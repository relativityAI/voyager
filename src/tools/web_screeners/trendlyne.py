...
class Trendlyne(object):
    ...
    def _flatten_data(self, data):
        ...
        for label, score_dict in swot.items():
            flat_data[f"SWOT {label} Score"] = self._parse_value(
                ...
                for item in p_data:
                    if isinstance(item, dict):
                        ...
                        if name:
                            ...
                            key = f"{prefix} {name}"
                            if "value" in item:
                                flat_data[key] = self._parse_value(item["value"])
                            elif "data" in item:
                                flat_data[key] = self._parse_value(item["data"])

                            if "changePercent" in item:
                                flat_data[f"{key} Change %"] = self._parse_value(
                                    item["changePercent"]
                                )
                            if "changePercentSafe" in item:
                                flat_data[f"{key} Change %"] = self._parse_value(
                                    item["changePercentSafe"]
                                )
            else:
    ...
    def fetch(self, symbol="KEI"):
        ...
        response = self.session.get(
            url, headers=generate_fake_headers(), allow_redirects=True
        )
        ...
        metrics_api_url = (
            f"https://trendlyne.com/equity/getStockMetricParameterList/{code}/"
        )
        ...
        technicals_url = (
            f"https://trendlyne.com/equity/technical-analysis/{symbol}/{code}/{slug}/"
        )
                ...
                for i in parameters[p]:
                    if type(i) == dict:
                        if "name" in i.keys():
                            ...
                            if "value" in i.keys():
                                output += str(i["value"])
                            if "changePercentSafe" in i.keys():
                                output += str(i["changePercentSafe"])
                            output += "\n"
